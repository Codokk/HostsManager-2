...
async function start1() {
    ...
    document.getElementById("applyProfilesButton").addEventListener("click", async () => {
        let sites = await getSitesPromise();
        // Generate Sites file
        let hostsString = "# Generated by HostsManager \n";
        sites.forEach((site)=>{
            hostsString += `${site.ip}\t${site.fqdn}\t${site.nickname}\n`;
        });
        let ret = await backend.writeHostsFile(N, hostsString);
        N.os.showMessageBox("Value", ret.success ? "Hosts file was updated successfully" : "Hosts file was not updated successfully \n Please contact cmkrist with this message.");
        console.log(ret);
    });
    // Set Events
    ...
    try {
        if(await N.storage.getData("Tray_notify") == "false") {
            ...
            backend.fixHostsFilePermission(N);
        }
    } catch {
        Notify_About_Tray = true;
    }
